...
export default function EditProfilePage() {
  ...
  const onSubmit = async (data: z.infer<typeof formSchema>) => {
    try {
      ...
      router.push(`/profile/${currentUser.data.username}`);
    } catch (err) {
      console.log(err);
        ...
        <CardContent>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="flex flex-col items-center">
              <label htmlFor="imageUpload" className="cursor-pointer group flex flex-col items-center">
                <div className="w-24 h-24 rounded-full overflow-hidden border-2 border-muted shadow-md hover:opacity-80 transition-all">
                  <Image src={previewImage || "/default-avatar.png"} alt="Profile" width={96} height={96} className="object-cover w-full h-full" />
                </div>
                <input type="file" accept="image/png" id="imageUpload" className="hidden" onChange={handleImageChange} />
            ...
            <div className="relative">
              <BadgeInfo className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" size={16} />
              <Input {...register("title")} placeholder="Title (e.g. Developer)" className="pl-10" />
              {errors.title?.message && <p className="text-sm text-red-500 mt-1">{errors.title.message}</p>}
            </div>

            {/* Bio */}
            <div className="relative">
              <User className="absolute left-3 top-4 text-muted-foreground" size={16} />
              <div>
                <Textarea
                  {...register("bio")}
                  value={bioValue}
                ...
                />
                <div className={`text-right text-xs mt-1 ${bioValue.length >= 200 ? "text-red-500" : "text-muted-foreground"}`}>{bioValue.length}/200 characters</div>
                {errors.bio?.message && <p className="text-sm text-red-500 mt-1">{errors.bio.message}</p>}
              </div>
            </div>
            ...
            <div className="relative">
              ...
              <Input {...register("country")} placeholder="Country" className="pl-10" />
              {errors.country?.message && <p className="text-sm text-red-500 mt-1">{errors.country.message}</p>}
            </div>

            {/* State */}
            <div className="relative">
              <MapPin className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" size={16} />
              <Input {...register("state")} placeholder="State" className="pl-10" />
              {errors.state?.message && <p className="text-sm text-red-500 mt-1">{errors.state.message}</p>}
            </div>

            {/* GitHub */}
            <div className="relative">
              <svg xmlns="http://www.w3.org/2000/svg" className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
              ...
              <Input {...register("socialLinks.github")} placeholder="GitHub URL" className="pl-10" />
              {errors.socialLinks?.github?.message && <p className="text-sm text-red-500 mt-1">{errors.socialLinks.github.message}</p>}
            </div>

            {/* LinkedIn */}
            <div className="relative">
              <svg xmlns="http://www.w3.org/2000/svg" className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
              ...
              <Input {...register("socialLinks.linkedin")} placeholder="LinkedIn URL" className="pl-10" />
              {errors.socialLinks?.linkedin?.message && <p className="text-sm text-red-500 mt-1">{errors.socialLinks.linkedin.message}</p>}
            </div>
            ...
            {currentUser?.data?.profileUpdated && (
              <Button type="button" variant="ghost" className="w-full border mt-2 cursor-pointer" onClick={() => router.push(`/profile`)}>
                Cancel
              </Button>
            )}
          </form>
